...
document.addEventListener("DOMContentLoaded", function () {
  ...
  braceletButtons.forEach(btn => {
    btn.addEventListener('click', () => {
      ...
      braceletSelect.value = value;  // синхронизировать скрытый select (если оставлен)
      updateLayer(braceletLayer, "bracelet", `Bracelet_${value}`);
      saveToLocal();
    });
  });

  // при старте: выделить нужную кнопку
  const currentBracelet = braceletSelect.value;
  braceletButtons.forEach(b => {
    if (b.dataset.value === currentBracelet) {
