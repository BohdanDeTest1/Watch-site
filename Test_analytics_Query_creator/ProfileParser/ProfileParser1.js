...
(function () {
  ...
  `;
    ...
    function appendLiveOpsTable(items) {
        ...
        wrap.className = 'pp-item';
        wrap.innerHTML = `
  <div class="pp-title">LiveOps</div>
...
<div class="pp-type-filter">
  ...
  <div class="pp-filter-pop pp-type-pop" id="ppTypePop" hidden>
    ...
    <!-- Start date + кнопка фильтра -->


<!-- End date + кнопка фильтра -->
<!-- ✅ Start Date + фильтр -->
<div class="pp-date-filter">
  <div class="pp-sort" data-key="startPretty">Start Date</div>
  <button id="ppStartBtn" class="pp-icon-btn" aria-label="Filter Start Date" title="Filter">
  </button>
...
<div class="pp-date-filter">
  ...
  <button id="ppEndBtn" class="pp-icon-btn" aria-label="Filter End Date" title="Filter">
  </button>
    ...
    async function runProfileLookup(name) {
        ...
        try {
            ...
            // 3. Promotions
            {
                ...
                appendResult('Промоушены получены', first10(t), t.length);
            }
        } catch (e) {
            const item = document.createElement('div');
            item.className = 'pp-item';
